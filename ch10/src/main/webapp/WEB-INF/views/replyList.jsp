...
<script type="text/javascript">
	...
	function rUpdate(bno,rno) {
		//input에 있는 데이터를 읽어서 textarea에 넣어서 변경 해야함
		//input이나 textarea(form tag안)에 있는 데이터를 읽을때 jquery에서는 val()
		//td에 있는 데이터(일반 tag안)를 읽을 때는 jquery에서는 text()
		var txt = $('#td_'+rno).text();
		//댓글 수정/삭제, 클릭한 위치에서 수정가능
		//댓글 처리
		$('#td_'+rno).html("<textarea rows='3' cols='30' id='rt'>"+txt+"</textarea>");
		//클릭한 위치에 있는 수정버튼을 확인과 취소로 변경
		//버튼 처리
